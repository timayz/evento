...
pub trait AggregateEvent: Aggregate {
    ...
    /// Older stored events that upcast — directly or through a chain — into
    /// this one.
    ///
    /// Generated by `#[evento::aggregate]` from `#[evento(upcast_to = ...)]`
    /// on the older variants. A [`Projection`](crate::projection::Projection)
    /// or [`SubscriptionBuilder`](crate::subscription::SubscriptionBuilder)
    /// that registers a handler (or a skip) for this event also accepts those
    /// older events, converted before the handler sees them.
    fn upcasters() -> &'static [Upcaster] {
        &[]
    }
}
...
///
/// Built by `#[evento::aggregate]`; one entry per older event, with the whole
/// chain (`V1 -> V2 -> V3`) folded into a single decode and encode.
#[derive(Debug, Clone, Copy)]
#[non_exhaustive]
pub struct Upcaster {
    /// Stored name of the older event.
    pub from: &'static str,
    /// Number of `upcast_to` steps between `from` and the target event. When
    /// an old event could reach several registered handlers (`V2` and `V3`
    /// both handled), the nearest one wins.
    pub hops: u8,
    /// Decodes the older payload, converts it, and encodes the target payload.
    pub upcast: fn(&[u8]) -> Result<Vec<u8>, bitcode::Error>,
}
...
impl Upcaster {
    ...
    pub const fn new(
        ...
        Self { from, hops, upcast }
    }
}

/// Conversion used by `#[evento(upcast_to = New)]`: implemented for every
/// `New: From<Old>`.
///
/// It only exists to turn a missing `From` impl into a readable error on the
/// `upcast_to` attribute; implement [`From`], not this trait.
#[diagnostic::on_unimplemented(
    message = "event `{Old}` cannot be upcast to `{Self}`",
    label = "`From<{Old}>` is not implemented for `{Self}`",
    note = "add `impl From<{Old}> for {Self}`"
)]
pub trait UpcastFrom<Old>: Sized {
    /// Converts the older event into this one.
    fn upcast_from(old: Old) -> Self;
}

impl<New: From<Old>, Old> UpcastFrom<Old> for New {
    fn upcast_from(old: Old) -> Self {
        New::from(old)
    }
}
...
pub trait AggregateExt<E: Executor> {
    ...
    /// — or of an older event that [upcasts](AggregateEvent::upcasters) into it.
