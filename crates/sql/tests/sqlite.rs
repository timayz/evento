...
async fn create_rw_sqlite_pool(key: impl Into<String>) -> anyhow::Result<(SqlitePool, SqlitePool)> {
    ...
    // CARGO_TARGET_TMPDIR is an absolute, cargo-created dir for integration
    // tests — independent of the crate's location in the workspace.
    let url = format!("sqlite://{}/test_sql_{key}.db", env!("CARGO_TARGET_TMPDIR"));
