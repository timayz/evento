...
verification has begun**: an independent Jepsen/Elle harness (`crates/accord/jepsen/`,
...
- **Phase B — Durability & recovery (delivers the pluggable-storage goal).**
  ...
  (`crates/sql/tests/sql_journal{,_postgres,_mysql}.rs`, shared scenarios, gated by the
...
- **Phase D — Sign-off.** 🚧 *In progress.* ✅ **Observability:** a per-node
  ...
  **Phase D remaining (sign-off):**
  ...
  - [~] **External / adversarial verification — Jepsen suite.** 🚧 Harness built
        (`crates/accord/jepsen/`): a self-contained Docker cluster (control + n1..n5)
