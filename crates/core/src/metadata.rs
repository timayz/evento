...
impl Metadata {
    /// Builds metadata from a pre-existing id and raw key/value map.
    ///
    /// The `meta` values are stored verbatim (already-encoded bytes); no
    /// encoding is applied. This is the counterpart to the read-side access
    /// via [`Deref`] and exists so callers outside this crate (e.g. a server
    /// rebuilding metadata received over the wire) can reconstruct a
    /// `Metadata` with full fidelity, preserving arbitrary keys.
    pub fn from_parts(id: String, meta: HashMap<String, Vec<u8>>) -> Self {
        Self { id, meta }
    }
