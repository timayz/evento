...
pub fn subscription_next_impl(input: &ItemFn) -> syn::Result<TokenStream> {
    ...
    let _ = params.next();
    // Second param: Event<AccountOpened>
    let event_arg = params.next().ok_or_else(|| {
        Error::new_spanned(&input.sig, "expected second parameter: event: Event<T>")
    })?;
