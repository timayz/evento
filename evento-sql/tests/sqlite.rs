...
#[tokio::test]
async fn sqlite_subscription_data() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("subscription_data").await?;
...
#[tokio::test]
async fn sqlite_upcast_load() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("upcast_load").await?;
    evento_test::upcast_load(&executor).await
}

#[tokio::test]
async fn sqlite_upcast_nearest_target() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("upcast_nearest_target").await?;
    evento_test::upcast_nearest_target(&executor).await
}

#[tokio::test]
async fn sqlite_upcast_explicit_wins() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("upcast_explicit_wins").await?;
    evento_test::upcast_explicit_wins(&executor).await
}

#[tokio::test]
async fn sqlite_upcast_skip() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("upcast_skip").await?;
    evento_test::upcast_skip(&executor).await
}

#[tokio::test]
async fn sqlite_upcast_strict_unhandled_target() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("upcast_strict_unhandled_target").await?;
    evento_test::upcast_strict_unhandled_target(&executor).await
}

#[tokio::test]
async fn sqlite_upcast_subscription() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("upcast_subscription").await?;
    evento_test::upcast_subscription(&executor).await
}

#[tokio::test]
async fn sqlite_upcast_projection_subscription() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("upcast_projection_subscription").await?;
    evento_test::upcast_projection_subscription(&executor).await
}

#[tokio::test]
async fn sqlite_upcast_tombstone() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("upcast_tombstone").await?;
    evento_test::upcast_tombstone(&executor).await
}

#[tokio::test]
async fn sqlite_upcast_has_event() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("upcast_has_event").await?;
    evento_test::upcast_has_event(&executor).await
}

#[tokio::test]
async fn sqlite_read_stream() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("read_stream").await?;
    evento_test::read_stream(&executor).await
}

#[tokio::test]
async fn sqlite_read_drains_pages() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("read_drains_pages").await?;
    evento_test::read_drains_pages(&executor).await
}

#[tokio::test]
async fn sqlite_read_limit() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("read_limit").await?;
    evento_test::read_limit(&executor).await
}

#[tokio::test]
async fn sqlite_read_page_cursor() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("read_page_cursor").await?;
    evento_test::read_page_cursor(&executor).await
}

#[tokio::test]
async fn sqlite_read_decode() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("read_decode").await?;
    evento_test::read_decode(&executor).await
}

#[tokio::test]
async fn sqlite_read_routing_key() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("read_routing_key").await?;
    evento_test::read_routing_key(&executor).await
}

#[tokio::test]
async fn sqlite_subscription_start_from_latest() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("sub_start_from_latest").await?;
    evento_test::subscription_start_from_latest(&executor).await
}

#[tokio::test]
async fn sqlite_subscription_ephemeral() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("sub_ephemeral").await?;
    evento_test::subscription_ephemeral(&executor).await
}

#[tokio::test]
async fn sqlite_subscription_ephemeral_start_from_latest() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("sub_ephemeral_start_from_latest").await?;
    evento_test::subscription_ephemeral_start_from_latest(&executor).await
}

#[tokio::test]
async fn sqlite_subscription_ephemeral_concurrent_same_key() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("sub_ephemeral_concurrent_same_key").await?;
    evento_test::subscription_ephemeral_concurrent_same_key(&executor).await
}

#[tokio::test]
async fn sqlite_subscription_context_stop() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("sub_context_stop").await?;
...
#[tokio::test]
async fn sqlite_subscription_ephemeral_run_once() -> anyhow::Result<()> {
    let executor = create_sqlite_executor("sub_ephemeral_run_once").await?;
